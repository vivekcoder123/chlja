...
class ListItem extends Component {
  ...
  handlelikeUser = (e) => {
    ...
    .then((res) => {
      alert({
        success: 'Contact has been liked successfully!'
      });
      this.setState({loading:false});
    }).catch((error) => {
      alert({
  ...
  render() {
    if(this.state.loading){
      ...
      {console.log("pikachu",this.props.user)}
    const user = this.props.user;
    return (
      <div class="ui card" style={{width:"100%"}}>
  <div class="content">
    <i title="Edit Contact" style={{cursor:"pointer",fontSize:"24px",color:"#39b"}} class="right floated edit icon" onClick={this.handleEditUser}></i>
    <i title="Delete Contact" style={{cursor:"pointer",fontSize:"24px",color:"#f65d3c"}} 
    class="right floated trash icon" 
    data-toggle="modal" data-target={`#myModal${user.id}`}></i>
    <img alt="contact icon" class="ui avatar image" src="https://semantic-ui.com/images/avatar/large/elliot.jpg"/><strong>{user.title}</strong>
    
    <div class="description">
      <p>{user.description}</p>
      <h4 style={{margin:"0px"}}>Phone No:<a target="_blank" href='#'>{user.url}</a></h4>
    </div>
  </div>
  <div class="extra content">
    <span class="left floated like" onClick={this.handlelikeUser} style={{cursor:"pointer"}}>
       Status : 
       {console.log("prabhat",user.likes)}
      {user.likes===1?"Active":"Inactive"}
